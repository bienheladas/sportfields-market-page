...
// Renderiza una FieldSummary (agregada del hook useRentSlots por
// ownerNFTName). El cover es un SVG placeholder porque el OwnerRecord
// on-chain todavía no tiene imageCid — sustituible por <img> el día
// que se agregue al datum.

import * as React from 'react';
import type { FieldSummary } from './types.additions';
import { decodeBBS, shortenAddr } from './lib';
import { PriceAda } from './atoms';

export interface FieldCardProps {
  field: FieldSummary;
  /** Disparado al click — el padre navega a `/field/:ownerNFTName`. */
  onOpen?: (ownerNFTName: string) => void;
  /** Render compacto: oculta el cover, ideal para vistas densas. */
  compact?: boolean;
  className?: string;
}

// SVG gradients para variar el cover. Decididas a partir del
// ownerNFTName para que cada cancha mantenga su color entre renders.
...
function gradientFor(ownerNFTName: string): GradId {
  let h = 0;
  for (let i = 0; i < ownerNFTName.length; i++) {
    h = (h * 31 + ownerNFTName.charCodeAt(i)) >>> 0;
  }
...
function FieldCover({
  ...
  return (
    <div className="relative aspect-[16/10] bg-[var(--paper-2)] overflow-hidden">
      ...
      {/* tag mint en esquina superior izquierda */}
      ...
      {/* ownerNFTName en mono en esquina superior derecha */}
...
/** Picks a pair of stops based on ownerNFTName so the same field keeps
 *  the same look across renders. */
