// react/SlotFilterBar.tsx
// Hero search bar para FieldDiscovery — selector de día (Lun–Dom) y
// franja horaria (00–23) + botón "Buscar". Estado controlado por el
// padre; al hacer click en "Buscar" emite onSearch() (no mutation).
//
// Diseño: dos celdas tipo Airbnb dentro de un contenedor redondeado,
// cada una con un popover que se abre al click. Tailwind 3 + CSS vars
// del proyecto.
...
export interface SlotFilterBarProps {
  ...
  /** Disparado por el botón "Buscar". El padre puede usarlo para
   *  scrollear al grid o disparar un re-fetch. */
  onSearch?: () => void;
  className?: string;
}
            ...
            <div className="grid grid-cols-7 gap-1">
              {DAYS.map((d) => (
                <button
                  ...
                  className={[
                    'py-2.5 rounded-md text-[12px] font-semibold tracking-wider uppercase',
            ...
            <div className="grid grid-cols-6 gap-1">
              {HOURS.map((h) => (
                <button
                  ...
                  className={[
                    'py-1.5 rounded-md text-[11px] font-mono font-medium',
                    'border transition-colors',
                    value.hour === h
                      ? 'bg-[var(--ink)] border-[var(--ink)] text-[var(--paper)]'
                      : 'bg-[var(--paper)] border-[var(--line)] text-[var(--ink-2)] hover:border-[var(--ink-2)]',
  ...
  return (
    <div className="relative flex-1 min-w-0">
      ...
      >
        ...
        <span
          className={[
            'mt-1 block text-[15px] truncate',
            isPlaceholder ? 'text-[var(--muted)] font-normal' : 'text-[var(--ink)] font-medium',
          ].join(' ')}
        >
          {value}
        </span>
